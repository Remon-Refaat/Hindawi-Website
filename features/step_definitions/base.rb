...
# When
#############################
When /^clicking on "(.*)" button$/ do |button|
...
And /^I logged in with username "(.*)" with password "(\d+)"$/ do |username, password|
  ...
  step %Q{clicking on "Sign In" button}
end
...
And /^I upload a file to the field Manuscript File$/ do
  find("//input[@name='ManuscriptFile']").send_keys "#{ENV['DATA_DIR']}/test.docx"
end

And /^I upload a file to the field Optional Cover Letter$/ do
  find("//input[@name='CoverLetterReviewReport']").send_keys "#{ENV['DATA_DIR']}/test.docx"
end

And /^I upload a file to the field Optional Supplementary Materials$/ do
  find("//input[@name='SupplementaryMaterial']").send_keys "#{ENV['DATA_DIR']}/test.docx"
end
